...
public class OffsetLimitInterceptor implements Interceptor {
    ...
    public Object intercept(final Invocation invocation) throws Throwable {
        ...
        //手动将rowBounds设为null,关闭分页功能
        if (rowBounds == null){
            RowBounds noPaginate = new RowBounds();
            queryArgs[ROWBOUNDS_INDEX] = noPaginate;
            return invocation.proceed();
        }

        final PageBounds pageBounds = rowBounds instanceof OffsetPageBounds ? (OffsetPageBounds) rowBounds : new PageBounds(rowBounds);
        ...
        List result = new PageList(listFuture.get());
        Paginator paginator = new Paginator(pageBounds.getPage(), pageBounds.getLimit(), result.size());

        return new PageList<>(result, paginator);
    }
    ...
    // Reference: MapperBuilderAssistant

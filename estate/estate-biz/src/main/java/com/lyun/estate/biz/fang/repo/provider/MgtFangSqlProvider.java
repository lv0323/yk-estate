...
public class MgtFangSqlProvider {
    ...
    public String listSummary(MgtFangSelector selector) {
        return new SQL() {{
            SELECT(" f.id,  f.biz_type,  f.licence_id,  f.publish_price,  f.price_unit, f.unit_price, f.estate_area, f.real_area,  f.process, f.publish_time")
            ...
            if (SqlSupport.hasNotNullElement(selector.getDepartmentIds()) || nonNull(selector.getEmployeeId())) {
                LEFT_OUTER_JOIN("t_fang_info_owner fio ON f.id = fio.fang_id AND fio.is_deleted = FALSE");
            }
            ...
            WHERE_IF("f.house_type = #{houseType}", nonNull(selector.getHouseType()));
            WHERE_IF("f.resident = #{resident}", nonNull(selector.getResident()));
            WHERE_IF("f.estate_area >= #{minArea}", nonNull(selector.getMinArea()));
            WHERE_IF("f.estate_area < #{maxArea}", nonNull(selector.getMaxArea()));
            WHERE_IF("f.s_counts = #{sCounts}", nonNull(selector.getsCounts()));
            WHERE_IF("f.publish_price >= #{minPrice}", nonNull(selector.getMinPrice()));
            WHERE_IF("f.publish_price < #{maxPrice}", nonNull(selector.getMaxPrice()));
            WHERE_IF("fe.is_only = #{isOnly}", nonNull(selector.getIsOnly()));
            WHERE_IF("fe.over_years >= #{overYears}", nonNull(selector.getOverYears()));
            if (SqlSupport.hasNotNullElement(selector.getDepartmentIds())) {
                WHERE("fio.department_id IN (" + Joiner.on(",").skipNulls().join(selector.getDepartmentIds()) + ")");
            }
            WHERE_IF("fio.employee_id  = #{employeeId}", nonNull(selector.getEmployeeId()));
            WHERE_IF("fe.delegate_type  = #{delegateType}", nonNull(selector.getDelegateType()));
            WHERE_IF("fe.property_type  = #{propertyType}", nonNull(selector.getPropertyType()));
            ...
            WHERE_IF("t_ff.max_follow_time < #{maxFollowTime}", nonNull(selector.getMaxFollowTime()));
            WHERE_IF("c.near_line = #{nearLine}", nonNull(selector.getNearLine()));

...
public class FangFollowSqlProvider {
    public String listBySelector(FangFollowSelector selector) {
        SQL sql = new SQL().SELECT("ff.*,d.name AS department_name, e.name AS employee_name")
        ...
        if (hasNotNullElement(selector.getDepartmentIds())) {
            sql.WHERE("ff.department_id IN (" + Joiner.on(",").skipNulls().join(selector.getDepartmentIds()) + ")");
        }
        sql.WHERE_IF("ff.employee_id = #{employeeId}", nonNull(selector.getEmployeeId()));
        if (hasNotNullElement(selector.getIoDepartmentIds())) {
            sql.WHERE("fio.department_id IN (" + Joiner.on(",").skipNulls().join(selector.getIoDepartmentIds()) + ")");
        }
        sql.WHERE_IF("fio.employee_id = #{ioEmployeeId}", nonNull(selector.getIoEmployeeId()));
        sql.ORDER_BY("ff.id desc");
        return sql.toString();
    }
}

 ...
 */
require(['main-app',
        contextPath + '/js/service/regionAgent-service.js',
        ...
        'chosen'],
    function (mainApp, RegionService, FranchiseeService, UtilService, CityService, pagingPlugin, SweetAlertHelp) {
        ...
        var RegionAgentListModule=angular.module('RegionAgentListModule',['directiveYk']);
        RegionAgentListModule.controller("RegionAgentListCtrl", ['$scope','$timeout', '$q', '$interval', '$window', '$location', function ($scope, $timeout, $q, $interval, $window) {
            ...
            $scope.list = function (offset, currentPage) {
                ...
                FranchiseeService.listCompany($scope.filter, {'X-PAGING':'total=true&offset='+(offset||pageConfig.offset)+'&limit='+ pageConfig.limit}).then(function (res) {
                    $scope.$apply(function() {
                        $scope.regionAgentList = res.items.map(function (item) {
                            return {
                                id: item.id,
                                name: item.name,
        ...
        angular.element(document).ready(function() {
            angular.bootstrap(document.getElementById("regionAgentListWrapper"),["RegionAgentListModule"])
        });
